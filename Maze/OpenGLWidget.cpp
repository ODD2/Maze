...
void OpenGLWidget::paintGL()
{
	...
	if(MazeWidget::maze!=NULL)
	{
		...
		*/
		odPerspective(MazeWidget::maze->viewer_fov, 1, 0.01, 200);
		...
		*/
		...
		glMatrixMode(GL_MODELVIEW);
		glLoadIdentity();
		odLookAt(viewerPosX, viewerPosZ, viewerPosY,
...
{
	Maze* maze = MazeWidget::maze;
	// 畫右邊區塊的所有東西
	glBegin(GL_QUADS);
	//for(int j = 0 ; j< maze->num_cells;j++)
	for (int i = 0; i < maze->num_edges; i++) {
		if (MazeWidget::maze->edges[i]->opaque)
		{
			glColor3f(maze->edges[i]->color[0], maze->edges[i]->color[1], maze->edges[i]->color[2]);
			float edgeStartX = MazeWidget::maze->edges[i]->endpoints[Edge::START]->posn[Vertex::X];
			float edgeStartY = MazeWidget::maze->edges[i]->endpoints[Edge::START]->posn[Vertex::Y];
			float edgeEndX = MazeWidget::maze->edges[i]->endpoints[Edge::END]->posn[Vertex::X];
			float edgeEndY = MazeWidget::maze->edges[i]->endpoints[Edge::END]->posn[Vertex::Y];
			glVertex3f(edgeStartX, 1, edgeStartY);
			glVertex3f(edgeStartX, -1, edgeStartY);
			glVertex3f(edgeEndX, -1, edgeEndY);
			glVertex3f(edgeEndX, 1, edgeEndY);
		}
	}
